    ...
    controller('CostGanttCtrl', ['MainActivityCategory', 'currentTrend', 'TrendId', 'ProjectTitle', 'UnitType', 'Vendor', 'SubActivityCategory', 'Category', 'GetActivity', '$http', '$q', '$state', '$scope', '$compile', 'Program', 'ProgramElement',
        ...
        function (MainActivityCategory, currentTrend, TrendId, ProjectTitle, UnitType, Vendor, SubActivityCategory, Category, GetActivity, $http, $q, $state, $scope, $compile, Program, ProgramElement,
            ...
            function ConfigScheduleGantt() {
                ...
                function bindSubCategory(phaseid, index, vId, ProjectID)
                {
                    
                    $http.get(serviceBasePath + "Request/SubActivityCategory/" + $scope.OrganizationID + "/" + phaseid + "/" + index + "/" + vId + "/" + ProjectID).then(function (response) {
                        var subCategory = response.data.result;
                        for (var i = 0; i < subCategory.length; i++) {
                            var obj = {};
                            obj.key = subCategory[i].SubCategoryDescription;
                ...
                $scope.scheduleGanttInstance.form_blocks["CustomDDMainCategory"] = {
                    ...
                    set_value: function (node, value, ev, config) {
                        ...
                        if ($(".gantt_cal_light .gantt_title").html() != 'Add' && !value)
                            return;
                        ...
                        {
                            if (value) {
                                $("#ddMainCategory").val(value).trigger("chosen:updated");
                                //var main = $scope.scheduleGanttInstance.getLightboxSection('mainphase');
                                var sub = $scope.scheduleGanttInstance.getLightboxSection('subphase');
                                ...
                                for (var i = 0; i < MainCategory.length; i++) {
                                    ...
                                    if (value === MainCategory[i].CategoryDescription) {
                                ...
                                var ProjectID = $scope.x.ProjectID;
                                await bindSubCategory(phaseid, index, vId, ProjectID);
                            }
                        }
                        bindCategory();
                        ...
                        $('#ddMainCategory').unbind().chosen().change(function (e) {
                            ...
                            $scope.subCategory = [];
                            for (var i = 0; i < MainCategory.length; i++) {
                                if (selected === MainCategory[i].CategoryDescription) {
                                    index = MainCategory[i].CategoryID;
                                    vId = MainCategory[i].VersionId;
                                }
                            }
                            ...
                            $http.get(serviceBasePath + "Request/SubActivityCategory/" + $scope.OrganizationID + "/" + phaseid + "/" + index + "/" + vId + "/" + $scope.x.ProjectID).then(function (response) {
                                var subCategory = response.data.result;
                                for (var i = 0; i < subCategory.length; i++) {
                                    var obj = {};
                                    obj.key = subCategory[i].SubCategoryDescription;
                                ...
                                $('#ddSubCategory').val('').trigger("chosen:updated");
                ...
                $scope.scheduleGanttInstance.form_blocks["CustomDDSubCategory"] = {
                    ...
                    set_value: function (node, value, ev, config) {
                        ...
                        if ($scope.subCategory.length > 0)
                        {
                            ...
                            //    $('#ddSubCategory').val(value).trigger("chosen:updated");
