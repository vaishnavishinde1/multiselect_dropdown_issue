angular.module('cpp.controllers').
    controller('CertifiedPayrollCtrl', ['$http', 'Page', '$state', 'UpdateCertifiedPayroll', '$uibModal', '$scope', '$rootScope', 'TrendStatus', '$location', '$timeout', 'CertifiedPayroll',
        function ($http, Page, $state, UpdateCertifiedPayroll, $uibModal, $scope, $rootScope, TrendStatus, $location, $timeout, CertifiedPayroll) {
            var okToExit = true;
            Page.setTitle('Certified Payroll');
            ...
            var url = serviceBasePath + 'Response/CertifiedPayroll/';
            CertifiedPayroll.get({}, function (response) {
                console.log(response)
                $scope.checkList = [];
                $scope.CertifiedPayrollCollection = response.result;
                console.log($scope.CertifiedPayrollCollection);
                $scope.orgCertifiedPayrollCollection = angular.copy(response.result);
                addIndex($scope.CertifiedPayrollCollection);
                angular.forEach($scope.CertifiedPayrollCollection, function (item, index) {
                    $scope.checkList[index + 1] = false;
                    item.checkbox = false;
                });
                console.log($scope.CertifiedPayrollCollection);
                $scope.gridOptions.data = $scope.CertifiedPayrollCollection;
            });
            ...
            $scope.addRow = function () {
                var x = Math.max.apply(Math, $scope.CertifiedPayrollCollection.map(function (o) {
                ...
                $scope.CertifiedPayrollCollection.splice(x, 0, {
                    displayId: x,
                    Description: '',
                    checkbox: false,
                    new: true
            ...
            $scope.save = function () {
                ...
                angular.forEach($scope.CertifiedPayrollCollection, function (value, key, obj) {
                    if (!isInvalidList) {
                        //if (!Number.isInteger(parseFloat(value.ID))
                        //    || Number.parseFloat(value.ID) < 0) {
                        //    dhtmlx.alert('Department code must be integer. (Row ' + value.displayId + ')');
                        //    isInvalidList = true;
                        //    return;
                        //}
                        ...
                        if (value.new === true) {
                            ...
                            okToExit = false;
                            isChanged = true;
                            angular.forEach($scope.orgCertifiedPayrollCollection, function (orgItem) {
                ...
                if (isFilled == false) {
                    ...
                    $http({
                        ...
                        $scope.orgCertifiedPayrollCollection = angular.copy($scope.CertifiedPayrollCollection);
                        okToExit = true;
                        $state.reload();
            ...
            $scope.delete = function () {
                ...
                $scope.listToDelete = [];
                console.log($scope.CertifiedPayrollCollection);
                angular.forEach($scope.CertifiedPayrollCollection, function (item) {
                    isChecked = false;
                ...
                if (newList.length != 0) {
                    for (var i = 0; i < newList.length; i++) {
                        ...
                        if (ind != -1) {
                            ...
                            $scope.CertifiedPayrollCollection.splice(ind, 1);
                        }
                    }
            ...
            $scope.checkForChanges = function () {
                var unSavedChanges = false;
                var originalCollection = $scope.orgCertifiedPayrollCollection;
                var currentCollection = $scope.CertifiedPayrollCollection;
