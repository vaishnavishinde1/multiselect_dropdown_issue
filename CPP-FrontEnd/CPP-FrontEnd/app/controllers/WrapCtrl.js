angular.module('cpp.controllers').
    controller('WrapCtrl', ['$http', 'Page', '$state', 'UpdateWrap', '$uibModal', '$scope', '$rootScope', 'TrendStatus', '$location', '$timeout', 'Wrap',
        function ($http, Page, $state, UpdateWrap, $uibModal, $scope, $rootScope, TrendStatus, $location, $timeout, Wrap) {
            var okToExit = true;
            Page.setTitle('Wrap');
            ...
            var url = serviceBasePath + 'Response/Wrap/';
            Wrap.get({}, function (response) {
                console.log(response)
                $scope.checkList = [];
                $scope.WrapCollection = response.result;
                console.log($scope.WrapCollection);
                $scope.orgWrapCollection= angular.copy(response.result);
                addIndex($scope.WrapCollection);
                angular.forEach($scope.WrapCollection, function (item, index) {
                    $scope.checkList[index + 1] = false;
                    item.checkbox = false;
                });
                console.log($scope.WrapCollection);
                $scope.gridOptions.data = $scope.WrapCollection;
            });
            ...
            $scope.addRow = function () {
                var x = Math.max.apply(Math, $scope.WrapCollection.map(function (o) {
                ...
                $scope.WrapCollection.splice(x, 0, {
                    displayId: x,
                    Description: '',
                    checkbox: false,
                    new: true
            ...
            $scope.save = function () {
                ...
                angular.forEach($scope.WrapCollection, function (value, key, obj) {
                    if (!isInvalidList) {
                        ...
                        if (value.new === true) {
                            ...
                            okToExit = false;
                            isChanged = true;
                            angular.forEach($scope.orgWrapCollection, function (orgItem) {
                ...
                if (isFilled == false) {
                    ...
                    $http({
                        ...
                        $scope.orgWrapCollection = angular.copy($scope.WrapCollection);
                        okToExit = true;
                        $state.reload();
            ...
            $scope.delete = function () {
                ...
                $scope.listToDelete = [];
                console.log($scope.WrapCollection);
                angular.forEach($scope.WrapCollection, function (item) {
                    isChecked = false;
                ...
                if (newList.length != 0) {
                    for (var i = 0; i < newList.length; i++) {
                        ...
                        if (ind != -1) {
                            ...
                            $scope.WrapCollection.splice(ind, 1);
                        }
                    }
            ...
            $scope.checkForChanges = function () {
                var unSavedChanges = false;
                var originalCollection = $scope.orgWrapCollection;
                var currentCollection = $scope.WrapCollection;
