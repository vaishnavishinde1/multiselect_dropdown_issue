angular.module('cpp.controllers').
    controller('UserReportManagerCtrl', ['$scope', '$timeout', '$uibModal', '$rootScope', '$http', 'Program', 'ProgramElement', 'Page', 'Project', 'Trend', 'MaterialCategory', 'SubcontractorType', 'fteposition', 'Organization', 'PhaseCode', 'ProjectClass', 'ProjectClassPhase', 'VersionDetails', 'Category',
        'Client',
        function ($scope, $timeout, $uibModal, $rootScope, $http, Program, ProgramElement, Page, Project, Trend, MaterialCategory, SubcontractorType, fteposition, Organization, PhaseCode, ProjectClass, ProjectClassPhase, VersionDetails, Category, Client) {

            Page.setTitle('User Report Manager');
            ...
            $scope.reportTypeList = [
                ...
                { filterName: 'Contract Report', reportPathName: 'ContractReport', fileName: 'Contract Report' + '_' + + $scope.filedateformat,  filterLess: false },
            ...
            $scope.generateReport = function () {
                ...
                if ($scope.reportTypeFilter.filterLess) {   //Filterless Reports Report - MySQL
                    ...
                    baseUrl = serviceBasePath + 'Request/ContractReport';
                    debugger;
                    var pdfUrl = baseUrl
            ...
            function findFilterValues() {
                ...
                debugger;
                //Process client
                if ($scope.selectedClient != undefined && $scope.selectedClient != null && $scope.selectedClient.ClientID) {
                    allFilters.clientID = $scope.selectedClient.ClientID;
                } else {
                    allFilters.clientID = 0;
                }
            ...
            function initializeFilterLists() {
                ...
                Client.get({}, function (response) {
                    console.log(response)
                    $scope.allClientList = response.result;
                });
