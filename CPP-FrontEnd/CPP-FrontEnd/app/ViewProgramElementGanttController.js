 ...
 */
angular.module('xenon.ViewProgramElementGanttController', []).
    controller('ViewProgramElementGanttCtrl', ['MainActivityCategory', 'currentTrend', 'TrendId', 'ProjectTitle', 'UnitType', 'Vendor', 'SubActivityCategory', 'Category', 'GetActivity', '$http', '$q', '$state', '$scope', '$compile', 'Program', 'ProgramElement',
        ...
        'LaborRate', 'PurchaseOrder', 'PurchaseOrderDetail', 'MFAConfiguration', 'ViewProjectGanttActivities','ViewGanttCost',
        function (MainActivityCategory, currentTrend, TrendId, ProjectTitle, UnitType, Vendor, SubActivityCategory, Category, GetActivity, $http, $q, $state, $scope, $compile, Program, ProgramElement,
            ...
            ProjectTitle.setTitle(delayedData[8].result[0].ProgramElementName);    //07-01-2021
            ...
            var programElement = {};
            programElement["id"] = programElementMaxId;
            programElement["text"] = delayedData[8].result[0].ProgramElementName;
            ...
            programElement["percentage_completion"] = getProjectPercentageCompletion(projects);
            programElement["totalBudget"] = trendTotalBudget;
            ...
            setProgramElementOriginalStartEndDate(activities);
            //Manasi 13-01-2021
            ...
            function setProgramElementOriginalStartEndDate(activities) {
                var maxDate = new Date(8640000000000000);
                var minDate = new Date(-8640000000000000);
                var originalStartDate = maxDate;
                var originalEndDate = minDate;
                ...
                //}
                programElement["originalStartDate"] = originalStartDate;
                programElement["originalEndDate"] = originalEndDate;
            }
            ...
            $scope.scheduleGanttInstance.attachEvent("customSelect", function (id, phaseID, isPhase) {
                ...
                if ($scope.selectedActivity) {
                    ...
                    if (testGetParent === programElementMaxId && numberOfChildren === false) {
                        $scope.costGanttInstance.clearAll();
                        $('#total_label').hide();
                        $('#total_cost').hide();
                    } else {
                        ...
                        if (Number(id) == programElementMaxId) {
                            var phase = 'null';
                            var activity = 'null';
                        }
                        ...
                        //priteshs1
                        if (!isPhase && phaseID == 0 && Number(id) != programElementMaxId) {
                            //  alert(id);
                        ...
                        $scope.x = {
                            ...
                            Phasecode: phase,
                            ActivityID: activity,
                            ProgramElementID: delayedData[8].result[0].ProgramElementID
                        }
                        ...
                        if ($scope.isRequestCost == false && false) {   //luan here temporarily - switching
                            ...
                            console.log(Costobj);
                            var path = serviceBasePath + 'Request/ProjectCost/' + delayedData[2].result[0].ProjectID + '/' +
                                delayedData[3] + '/' + phase + '/' + activity + '/' +
                                $scope.scheduleScale + '/' + $scope.selectedActivity.BudgetID + '/' + $scope.localStorageSrevice.get('authorizationData').acl[15] + '/' +
                                $scope.selectedActivity.BudgetCategory + '/' + $scope.selectedActivity.BudgetSubCategory + '/' + delayedData[8].result[0].ProgramElementID;
                            $http.get(path).then(function success(CostData) {
                                //luan quest 2/14
            ...
            function initializePhases(phases) {
                ...
                for (var j = 0; j < phases.length; j++) {
                    ...
                    if (projectActivities.length != 0) {
                        phase["id"] = (Math.max.apply(Math, projectActivities.map(function (a) { return a.ActivityID })) + 1) * 1000;
                        maxProjectActId = (Math.max.apply(Math, projectActivities.map(function (a) { return a.ActivityID })) + 1);
                    }
                    else {
                        phase["id"] = (maxProjectActId + Number(phases[j].Order)) * 1000;
                    }
                    ...
                    phase["originalStartDate"] = "" + "";
                    phase["originalEndDate"] = "" + "";
